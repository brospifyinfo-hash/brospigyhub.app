...
export default async function AdminChannelEditPage({
  ...
  const { data: channel } = await supabase
    .from('channels')
    .select('id, name, category_id, sort_order, allow_text, allow_images, allow_user_images, show_download_button, show_copy_button, allow_anyone_to_post, requires_approval, history_visible, cta_text, cta_url')
    .eq('id', id)
    .single();
  if (!channel) notFound();
  ...
  return (
    <div className="max-w-2xl">
      <Link
        href="/admin/channels"
        className="mb-4 inline-block text-sm text-[var(--color-accent)]"
      >
        ← Alle Channels
      </Link>
      ...
      <form action={deleteChannel.bind(null, id)} className="mt-6">
        <button
          type="submit"
          className="rounded-xl border border-red-500/50 px-4 py-2 text-red-400 hover:bg-red-500/10"
        >
          Channel löschen
        </button>
      </form>
