...
export async function saveChannelCategory(
  _prev: State,
  formData: FormData
): Promise<State> {
  ...
  revalidatePath('/admin/channel-categories');
  redirect('/admin/channel-categories?saved=1');
}
...
export async function deleteChannelCategory(id: string) {
  ...
  revalidatePath('/admin/channel-categories');
  redirect('/admin/channel-categories?deleted=1');
}
