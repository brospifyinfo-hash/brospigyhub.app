  ...
  return (
    <>
      <button
        ...
        className="absolute left-3 top-3 z-20 flex h-11 w-11 items-center justify-center rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-bg-dark)]/90 text-[var(--color-text)] shadow-lg backdrop-blur-xl transition-transform active:scale-95 md:hidden"
      ...
      {mobileOpen && (
        <>
          <div
            ...
            aria-hidden
          />
          <div className="fixed inset-y-0 left-0 z-50 w-[min(320px,85vw)] overflow-y-auto border-r border-[var(--glass-border)] bg-[var(--glass-bg-dark)] p-5 shadow-2xl md:hidden">
            {drawerNav}
          </div>
        </>
      )}
        ...
        <div className="p-4">
          ...
          {byCategory.map((cat) => (
            <div key={cat.id} className="mb-4">
              ...
              <div className="space-y-0.5">
                {cat.channels.map((ch) => (
                  <Link key={ch.id} href={`/dashboard/channels/${ch.id}`} className={linkClass(ch.id === channelId)}>
                    {ch.name}
                  </Link>
                ))}
              </div>
