...
export async function loginWithLicenseKey(_prev: State, formData: FormData): Promise<State> {
  try {
    ...
    if (missingEnv.length > 0) {
      return {
        error: `Server-Konfiguration unvollständig (${missingEnv.join(', ')}). Bitte Vercel Environment Variables prüfen.`,
      };
    }
    ...
    const msg = err instanceof Error ? err.message : 'Unbekannter Login-Fehler';
    return { error: `Login fehlgeschlagen: ${msg}` };
  }
}
